...
/* User.findOne({UserID:checkoutdetails.user}).exec(function (err,userDet) {
 if(err)
 {
 console.error(err);
 callback2();
 ...
 {
 Membership.findById(userDet.membershipId).exec(function (err,membership) {
 if(err)
 {
 console.error(err);
 callback2();
 ...
 if(!membership)
 {
 console.error('No membership found by this id');
 callback2();
 }
 ...
 if (Number(duration) <= Number(eMemberships[i].plans[j].endTime)) {
 ...
 CheckOut.findByIdAndUpdate(checkoutdetails._id,{$set:{localUpdate:1}},{new:true}).exec(function (err,result) {
 if(err)
 {
 console.error(err);
 callback2();
 }
 checkin.findByIdAndUpdate(checkinDetail._id,{$set:{localUpdate:1}},{new:true}).exec(function (err,result) {
 if(err)
 {
 console.error(err);
 callback2();
 ...
 User.findByIdAndUpdate(userDet._id,{$set: {vehicleId: userDet.vehicleId}}, {new: true}).exec(function (err, updatedUser) {
 if (err) {
 console.error(err);
 callback2();
 }
            ...
            if(checkinDetails.length>0)
            {
                async.forEach(checkinDetails,function (checkinDetail) {
                    CheckOut.findOne({
                        ...
                        'localUpdate':0,
                        'status':'Close',
                        'checkOutTime': {$gte:moment(today),$lt:moment(checkinDetail.checkInTime)}
